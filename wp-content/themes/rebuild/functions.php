...
function rebuild_setup() {
	/*
		...
		* If you're building a theme based on rebuild, use a find and replace
		* to change 'rebuild' to the name of your theme in all the template files.
		*/
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'rebuild_custom_background_args',
			array(
 ...
 */
function rebuild_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'rebuild_content_width', 640 );
}
add_action( 'after_setup_theme', 'rebuild_content_width', 0 );
 ...
 */
function rebuild_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'rebuild' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'rebuild' ),
...
add_action( 'widgets_init', 'rebuild_widgets_init' );
...
function rebuild_scripts() {
	...
	wp_enqueue_style( 'main-style', get_template_directory_uri(). '/css/main.css?v.400.6', array(), _S_VERSION );
    ...
    wp_enqueue_script( 'main-script', get_template_directory_uri() . '/js/main.js?v.118.4', array(), _S_VERSION, true );
